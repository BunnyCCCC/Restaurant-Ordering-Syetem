...
Instruction:
    1.download the file assignment4.zip and unzip it.
    2.make sure the mongodb is running using command:
    ...
    4. once all these are done, we can run the server using command:
        node server.js
       and you will see a message: Server running at http://127.0.0.1:3000/
    5. once server is successful, open the browser of your choice and navigate to http://127.0.0.1:3000/ and you
       will see the welcome login page displayed.
    6. The top of the page is the header which allows you to navigate through home, users, login and register.
    7. users tab showing all the current users stored in the database a4 the users collection, you can click the page to the specific
       users and will be able to change the privacy mode for that users.
    8. Once the submit changes button is clicked, all the changes made will be sent to server and gets updated in the database.
    9. register tab allow you to add users with unique ID, and redirect it to the /users/:uid page to edit as loggedin user.
    10.Once the user loggedin, it can choose to changed its own profile privacy mode and it will be stored in the database. It can also
       choose to place order and saved to the database a4 under orders collection.
    11.The logout tab will log user out from the system, and all those private profiles and orders will not be accessed by user.
    12.use command "control+c" to interrupt the running server process and all new users and orders added will still be stored in the
       database a4.
    13.If you want to reset the database, rerun
        node database-initializer.js
